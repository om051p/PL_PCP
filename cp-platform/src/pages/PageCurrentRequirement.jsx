...
export function PageCurrentRequirement() {
  const stations =
    useProjectStore(
      (s) => s.projects.find((p) => p.id === s.activeProjectId)?.stations ?? s.projects[0]?.stations ?? []
    )
  const calculateStation = useProjectStore((s) => s.calculateStation)
  const project =
    useProjectStore((s) => s.projects.find((p) => p.id === s.activeProjectId) || s.projects[0])
  ...
  const totalDesignA = stations.reduce((acc, st) => acc + (st.lastCalcResult?.designCurrentA || 0), 0)
  ...
  return (
    <div className="page">
      ...
      <DesignStandardInfoBox project={project} std={std} tc={tc} spareFactor={spareFactor} formula={formula} />
            ...
            >
              ...
              {r ? (
                <>
                  ...
                  <FormulaCardGroup
                    title="Current Requirement Formulas"
                    standardRef={std.label}
                  >
                    <FormulaCard
                      name="Surface Area"
                      equation="A = π × D × L"
                      standardRef={std.label}
                      variables={st.pipelineSegments.map((seg) => ({
                        symbol: 'D',
