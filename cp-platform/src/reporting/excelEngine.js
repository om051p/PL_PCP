...
import { ANODE_SPECS, getActiveStandard } from '../constants/index.js'
...
export function exportToExcel(project) {
  ...
  const summaryData = [
    ...
    ['Design Standard', `${activeStd.label} (${activeStd.version})`],
  ...
  for (const st of project.stations) {
    ...
    const rows = [
      ...
      ...(st.groundbed.type === 'deepwell'
        ? [
            ['Coke Cover', st.groundbed.cokeCoverM, 'm'],
            ['Cement Plug', st.groundbed.cementPlugM, 'm'],
          ]
        : []),
