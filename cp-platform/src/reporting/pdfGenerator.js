...
function drawFooter(doc, project) {
  ...
  doc.text(`Client: ${project.clientName || ''}  |  ${project.endClient || ''}`, PW / 2, PH - 4.5, { align: 'center' })
...
function bomTable(doc, y, bom) {
  ...
  bom.forEach((item, i) => {
    ...
    const desc = item.description.length > 68 ? item.description.substring(0, 65) + '...' : item.description
...
export function generateEngineeringReport(project) {
  ...
  setFont(doc, 7, 'bold', C.white)
  let cx = ML + 2
  headers.forEach((h, i) => { doc.text(h, cx, y + 4); cx += smCols[i] })
  y += 6
  ...
  y = kvTable(doc, y, [
    ['NACE SP0169',     'Control of External Corrosion on Underground Metallic Piping Systems'],
    ['17-SAMSS-003',    'Transformer-Rectifier Units for Cathodic Protection'],
    ['17-SAMSS-016',    'Cast Iron Anodes for Impressed Current Systems'],
    ['17-SAMSS-008',    'Junction Boxes for Cathodic Protection Systems'],
  ...
  for (const station of project.stations) {
    ...
    doc.text(`Design Mode: ${station.groundbed.type === 'deepwell' ? 'Deepwell ICCP' : 'Shallow Vertical ICCP'}`, ML + CW - 3, y + 6.2, { align: 'right' })
    y += 13
    ...
    y = kvTable(doc, y, [
      ...
      ['Groundbed Remoteness',     `${station.actualRemotenesM} m`, `(Required: ≥${station.requiredRemotenesM} m)`],
    ])
    ...
    if (r) {
      y = kvTable(doc, y, [
        ...
        ['Temp-Corrected Current Density', `${f4(r.tempCorrectedCurrentDensity)} mA/m²`, 'i_T = i_base × [1+(T-25)×0.025]'],
      ...
      doc.text('Station not calculated.', ML + 3, y + 5); y += 10
    }
    ...
    const gbRows = [
      ['Groundbed Type',       isDeep ? 'Deepwell (Single Borehole)' : 'Shallow Vertical (Multiple Holes)'],
    ...
    if (r) {
      gbRows.push(['Active Column Length',    `${f2(r.activeLengthM)} m`, isDeep ? 'L = N×L_a + (N-1)×S' : ''])
      if (isDeep) gbRows.push(['Total Drilling Depth', `${f2(r.totalDrillDepthM)} m`])
      gbRows.push(['Groundbed Resistance R_G', `${f4(r.groundbedResistanceOhm)} Ω`, isDeep ? 'Dwight formula' : 'Sunde formula'])
    ...
    if (r) {
      y = kvTable(doc, y, [
        ...
        ['Parallel Anode Tail Resistance', `${f4(r.anodeTailParallelResOhm)} Ω`, 'R_ac = 1/Σ(1/L_i×r)'],
        ['Positive Main Cable',            `${station.cables.posMainLengthM} m × ${station.cables.posMainSizeMm2}mm²`],
        ['Positive Main Resistance',       `${f4(r.posMainCableResOhm)} Ω`],
        ['Negative Main Cable',            `${station.cables.negMainLengthM} m × ${station.cables.negMainSizeMm2}mm²`],
        ['Negative Secondary Cable',       `${station.cables.negSecLengthM} m × ${station.cables.negSecSizeMm2}mm²`],
    ...
    if (r) {
      y = kvTable(doc, y, [
        ...
        ['Total Circuit Resistance',  `${f4(r.totalCircuitResistanceOhm)} Ω`, 'R_T = R_G + R_c + R_emf + R_s'],
    ...
    if (r?.checks?.length) {
      ...
      setFont(doc, 8, 'italic', C.lightGray)
      doc.text('Station not calculated — no validation results.', ML + 3, y + 5); y += 10
    }
  }
