...
import { THRESHOLDS, getSoilClassification } from '../../constants/index.js'
...
const VALIDATION_RULES = [
  ...
  // BR-001: TR Voltage Adequate
  {
    ...
    check: (station, result) => {
      ...
      return {
        ...
        recommendation: pass ? null : `Increase TR to minimum ${Math.ceil(result.minTRVoltage / 5) * 5}V rating.`,
      }
    },
  ...
  // BR-002: Groundbed Resistance Within Allowable
  {
    ...
    insight: (station, result) => {
      ...
      const excess = ((result.groundbedResistanceOhm / result.maxAllowableGroundbedRes - 1) * 100).toFixed(0)
      return {
        severity: 'error',
        ...
        recommendations: [
          ...
          station.soilResistivityOhmCm > THRESHOLDS.HIGH_SOIL_RESISTIVITY
  ...
  // BR-003: Total Circuit Resistance < 70% Operating Threshold
  {
    ...
    insight: (station, result) => {
      ...
      return {
        severity: result.totalCircuitResistanceOhm >= result.maxCircuitRes70pct ? 'error' : 'warning',
  ...
  // BR-004: Circuit Resistance < 90% Warning Threshold
  {
    id: 'BR-004',
    check: (station, result) => {
      const status = result.totalCircuitResistanceOhm >= result.maxCircuitRes90pct ? 'fail'
        : result.totalCircuitResistanceOhm >= result.maxCircuitRes70pct ? 'warning' : 'pass'
      return {
        id: 'BR-004',
  ...
  // BR-005: Design Life Meets Target
  {
    ...
    check: (station, result) => {
      ...
      const warn = result.designLifeYears >= target && result.designLifeYears < target + THRESHOLDS.MIN_DESIGN_LIFE_MARGIN_Y
      return {
        id: 'BR-005',
        ...
        limit: `≥ ${target} years`,
        recommendation: pass ? (warn ? `Design life margin is only ${(result.designLifeYears - target).toFixed(1)} years. Consider adding 2 anodes for safety.` : null)
          : `Add anodes or increase anode weight to achieve ${target} year design life.`,
      }
    },
    insight: (station, result) => {
      const target = result.targetDesignLifeYears
      if (result.designLifeYears >= target + THRESHOLDS.MIN_DESIGN_LIFE_MARGIN_Y) return null
      const additionalAnodesNeeded = Math.ceil(
        (station.anodeSpec.consumptionRate * station.tr.ratedCurrent * target / station.anodeSpec.weightKg) - station.proposedAnodes + 1
      )
      return {
        severity: result.designLifeYears < target ? 'error' : 'warning',
        title: result.designLifeYears < target ? 'Design Life Insufficient' : 'Design Life Margin Low',
  ...
  // BR-006: Remoteness Check
  {
    id: 'BR-006',
    check: (station) => {
      const pass = station.actualRemotenesM >= station.requiredRemotenesM
      return {
        id: 'BR-006',
        label: `Groundbed Remoteness ≥ ${station.requiredRemotenesM}m`,
        status: pass ? 'pass' : 'fail',
        computed: `${station.actualRemotenesM}m actual`,
        limit: `≥ ${station.requiredRemotenesM}m`,
        recommendation: pass ? null : `Groundbed location must be moved to ≥${station.requiredRemotenesM}m from pipeline.`,
      }
    },
    insight: (station) => {
      if (station.actualRemotenesM >= station.requiredRemotenesM) return null
      return {
        severity: 'error',
        title: 'Groundbed Too Close to Pipeline',
        message: `Actual distance (${station.actualRemotenesM}m) is less than the required minimum (${station.requiredRemotenesM}m). This can cause current channelling and coating damage.`,
        recommendations: [
          `Relocate groundbed to achieve ≥${station.requiredRemotenesM}m separation.`,
...
const PROACTIVE_INSIGHT_RULES = [
  ...
  (station) => {
    ...
    if (rho < THRESHOLDS.HIGH_SOIL_RESISTIVITY) return null
    const classification = getSoilClassification(rho)
    return {
      severity: rho > THRESHOLDS.VERY_HIGH_SOIL_RESISTIVITY ? 'error' : 'warning',
      ...
      recommendations: [
        rho > THRESHOLDS.VERY_HIGH_SOIL_RESISTIVITY
 ...
 */
export function runRules(station, result) {
  const checks = VALIDATION_RULES.map(rule => rule.check(station, result))

  const ruleInsights = VALIDATION_RULES
    .map(rule => rule.insight(station, result))
    .filter(Boolean)

  const proactiveInsights = PROACTIVE_INSIGHT_RULES
    .map(fn => fn(station, result))
    .filter(Boolean)
